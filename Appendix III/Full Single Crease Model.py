...
os.chdir(r"N:\Project\ABAQUS\Results\SingleCrease_both sides")

PLoad = np.arange(10e-6,30.1e-4,10e-6)
#PLoad = [0.0007]
...
for P in PLoad:
	...
	# Variables
	MS = 0.06        # Mesh Size # 0.6 is the minimum amount that the simulation will work
	...
	mdb.models['Model-1'].Tie(adjust=ON, master=Region(
		...
		mask=('[#2 ]', ), )), thickness=ON, tieRotations=OFF)
	...
	mdb.models['Model-1'].rootAssembly.DatumCsysByThreePoints(coordSysType=
		...
		mdb.models['Model-1'].rootAssembly.referencePoints[6], point1=
	...
	mdb.models['Model-1'].rootAssembly.WirePolyLine(mergeType=IMPRINT, meshable=OFF
		, points=((
		mdb.models['Model-1'].rootAssembly.instances['Left Part-1'].vertices[1], 
		mdb.models['Model-1'].rootAssembly.instances['Right Part-1'].vertices[1]), 
		(mdb.models['Model-1'].rootAssembly.instances['Left Part-1'].vertices[2], 
		mdb.models['Model-1'].rootAssembly.instances['Right Part-1'].vertices[2])))
	...
	# Assign Sections	
	mdb.models['Model-1'].rootAssembly.ConnectorOrientation(region=mdb.models['Model-1'].
		rootAssembly.SectionAssignment(sectionName='REVOLUTE Section', 
		region=mdb.models['Model-1'].rootAssembly.sets['Wire Edges']).getSet(), 
		localCsys1=mdb.models['Model-1'].rootAssembly.datums[11])
	...
	mdb.models['Model-1'].rootAssembly.ReferencePoint(point=
		mdb.models['Model-1'].rootAssembly.datums[16])	
			
	# Create Load
	mdb.models['Model-1'].ShellEdgeLoad(createStepName='LoadApp', directionVector=(
		mdb.models['Model-1'].rootAssembly.instances['Right Part-1'].InterestingPoint(
		mdb.models['Model-1'].rootAssembly.instances['Right Part-1'].edges[3], 
		MIDDLE), mdb.models['Model-1'].rootAssembly.referencePoints[17]), 
		distributionType=UNIFORM, field='', localCsys=None, magnitude=P, name=
		'End Load', region=Region(
		side1Edges=mdb.models['Model-1'].rootAssembly.instances['Right Part-1'].edges.getSequenceFromMask(
		mask=('[#8 ]', ), )), resultant=ON, traction=GENERAL, follower=OFF)	
				
				
	# Job
	# Create
	# Create Job
	mdb.Job(name='SC2SidesAnalysis', model='Model-1', description='', 
		...
		numGPUs=0)	
	mdb.jobs['SC2SidesAnalysis'].setValues(numCpus=4, numDomains=4)	

	# Submit
	# Submit job	
	mdb.jobs['SC2SidesAnalysis'].submit(consistencyChecking=OFF)
	mdb.jobs['SC2SidesAnalysis'].waitForCompletion()

	# Extracting Results
	o3 = session.openOdb(
		name='N:\Project\ABAQUS\Results\SingleCrease_both sides\SC2SidesAnalysis.odb')
	#: Model: N:\Project\ABAQUS\Results\SingleCrease_both sides\SC2SidesAnalysis.odb
	...
	session.viewports['Viewport: 1'].setValues(
		displayedObject=session.odbs['N:\Project\ABAQUS\Results\SingleCrease_both sides\SC2SidesAnalysis.odb'])
	session.viewports['Viewport: 1'].assemblyDisplay.setValues(loads=OFF, bcs=OFF, 
		predefinedFields=OFF, connectors=OFF)
		
	# Save Deflection	
	odb = session.odbs['N:\Project\ABAQUS\Results\SingleCrease_both sides\SC2SidesAnalysis.odb']
	session.writeFieldReport(fileName='abaqusSC2sides'+str(P)+'.txt', append=OFF, 
	...
	session.printToFile(fileName='DeformedSC2sides'+str(P)+'.tif', format=TIFF, canvasObjects=(
